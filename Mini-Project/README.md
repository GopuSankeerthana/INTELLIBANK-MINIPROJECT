````md
# Finance ML Pipeline (IntelliBank) — Full Stack Finance Mini App
...
- Set **category budgets** and track spending vs limits
- Persistent SQLite database integration using Flask-SQLAlchemy
- CSV + Database hybrid storage support
- Multi-user transaction persistence
- Get **smart rule-based notifications**
- Receive **real-time email alerts using EmailJS**
...
This project includes lightweight **EmailJS integration** for real-time finance alert notifications.
...
4. Replace placeholders inside:

```text
static/js/email-alerts.js
```
...
### Note
For GitHub safety, placeholder keys are kept in the repository.
...
5) **Smart Notifications + Email Alerts**
   - Click **Notifications**
   - Explain rules:
   ...
   - Explain:
     - Important alerts can trigger real-time EmailJS notifications
...
- **Storage**:
  - CSV persistence for sample/demo datasets
  - SQLite database (finance.db) using Flask-SQLAlchemy
  - Hybrid persistence architecture (CSV + Database)
...
- **Notifications**: real-time email alerts using EmailJS integration

### Part D — Wrap-up (30 sec)
- Summarize outcomes:
  - “A mini finance app with ML insights, interactive charts, budgets, smart notifications, email alerts, and a finance health score.”
- Mention next steps:
...
### 4) Smart notifications (rule-based + EmailJS)
...
- Real-time email alerts using EmailJS integration
...
1) Transactions come from multiple persistence layers:
- CSVDataLoader reads:
  - data/sample_transactions.csv
  - data/sample_users.csv
- New transactions are persisted into:
  - SQLite database (finance.db)
  - CSV storage for demo continuity
- Flask APIs synchronize dashboard + storage layers
...
- EmailJS-powered alert notifications
...
### CSV Files
- `data/sample_transactions.csv` — sample transactions
- `data/sample_users.csv` — sample user profiles
- `data/budgets.csv` — category budgets per user
- `data/transactions.csv` — persisted transaction storage
- `data/users.csv` — persisted registered users

### SQLite Database
- `instance/finance.db`
  - db_user table
  - db_transaction table
  - db_budget table
...
The project now supports persistent SQLite database storage using Flask-SQLAlchemy.
...
- Multi-user support
- Dashboard data persistence after restart
- Hybrid CSV + SQLite architecture

### Initialization

The database initializes automatically when running:
...
- PostgreSQL/MySQL production migration
- User authentication system
- Export reports (PDF/Excel)
- Dark mode UI
- Stronger ML categorization model
